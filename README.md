...
Unit tests cover the core logic (schema normalization, condition evaluation, the
submission data contract) and run with the PHPUnit phar:

```bash
php phpunit.phar
```

CI lints `src/` and runs the suite on PHP 8.2–8.4. The Playwright end-to-end
suite and the dev data seeders are documented in [CONTRIBUTING.md](CONTRIBUTING.md).
